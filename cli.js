...
if (!checkPass) {
  error(
    `You are using Node ${chalk.cyan(process.version)}, but this version of ${chalk.cyan(
      pkg.name
    )} requires Node ${chalk.cyan(enginesNodeVersion)}.\r\nPlease upgrade your Node version`
  );
  process.exit(1);
}
...
program.version(`best-server ${chalk.cyan(pkg.version)}`, '-v --version').usage('[options]');
...
program
  ...
  .option(
    '-b, --base <base>',
    'Specify static directory root paths separated by commas (,) if multiple paths are used,eg:./'
  )
  ...
  .option(
    '-w, --watch <watch file>',
    'Specifies the folder or file to listen on. Subfolders are not supported. Use commas to separate multiple folders,eg:dist,./index.html'
  )
  .action((options) => {
    if (options.debug) {
...
program
  .command('config <init>')
  .description(
    'Initializes the configuration file to the specified directory, which is the current directory by default'
  )
...
program
  ...
  .description('Update the configuration file to the specified directory, which is the current directory by default')
  .argument('[directory]', 'Configuration file initialization directory address')
  .action((method, dir, options) => {
    initConfig({ method, dir, ...options });
  });
